...
\begin{enumerate}
    \item \textbf{Causal interpretation:} Despite our robustness exercises, we cannot definitively establish causality. Fertility is endogenous, and unobserved heterogeneity remains a concern. The Oster bounds ($\delta = 1.6$--$2.4$) suggest moderate to strong robustness, but do not rule out confounding.

    \item \textbf{CPS fertility measurement:} The mid-career estimates (ages 35--50) rely on co-resident children. Our within-CPS analysis (Table \ref{tab:cps_subgroups}) suggests this may create artifactual shifts in the reversal point as children leave home. The cross-cohort comparison within NLSY79 (ages 20--35 vs. 54--61) provides cleaner evidence.
    ...
    \item \textbf{Two-step residualization:} The shift in reversal point from OLS (38th percentile) to probit (42nd percentile) first stage (Table \ref{tab:first_stage}) is non-trivial, suggesting first-stage specification affects conclusions. Given the theoretical motivation for nonlinear selection, we present both specifications as bounding the true effect.

    \item \textbf{Decomposition interpretation:} The ``unexplained'' component captures all unobserved factors correlated with childlessness, not exclusively insurance mechanisms. Our interpretation of insurance effects at low quantiles is theoretically motivated but not uniquely identified.
\end{enumerate}
